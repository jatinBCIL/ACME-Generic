    ...
    public class clsDLCommon
    {
        ...
        public Boolean isLocked(string strUser)
        {
            ...
            try
            {
                objParameters[0] = new SqlParameter("@PARAM", SqlDbType.VarChar);
                objParameters[0].Value = strUser.Trim();
                if (_Sql.ExecuteScalar_Param(_Sql.strLocal, "SELECT ACTIVATE_ST FROM TBLMASTER_USER WHERE USER_ID = @PARAM", objParameters) == 1)
                {
                    return false;
                }
                else
        ...
        //    try
        //    {
        //        return objSql.ExecuteDataset(objSql.strLocal, "SELECT SRNO, MNUC, CAP, CHLD FROM TBLMASTER_USER_ROLES ORDER BY MNUC").Tables[0];
        //    }
        //    catch (Exception ex)
        //    {
        //        throw new Exception(ex.ToString());
        ...
        public DataSet getReportRights(string strUsername)
        {
            SqlDataLayer objSql = new SqlDataLayer();
            SqlParameter[] objParameters = new SqlParameter[1];
            try
            {
                ...
                objParameters[0].Value = strUsername;
                return objSql.ExecuteProcedureParamDataset(objSql.strLocal, "SP_GETReport", objParameters);
            }
            catch (Exception ex)
            {
                throw new Exception(ex.ToString());
        ...
        public DataTable BL_getModules()
        {
            ...
            try
            {
                return objSql.ExecuteDataset(objSql.strLocal, "SELECT MNUC, CAP, CHLD, SRNO FROM TBLMASTER_USER_ROLES ORDER BY SRNO").Tables[0];
            }
            catch (Exception ex)
            {
                throw new Exception(ex.ToString());
