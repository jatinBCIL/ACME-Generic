...
namespace DataLayer
{
    ...
    public class DL_BinMaster
    {
        ...
        public DataTable DL_GetBinDisplayData(string strWhere)
        {
            SqlDataLayer objSql = new SqlDataLayer();
            try
            {
                return objSql.ExecuteDataset(objSql.strLocal, "SELECT * FROM [VW_BINMASTER] " + strWhere.Trim()).Tables[0];
            }
            catch (Exception ex)
            {
                throw new Exception(ex.ToString());
        ...
        public string DL_Save_BinMaster(PL_BinMaster objFields)
        {
            SqlDataLayer objSql = new SqlDataLayer();
            SqlParameter[] objParameters = new SqlParameter[10];
            try
            {
                objParameters[0] = new SqlParameter("@LOCID", SqlDbType.VarChar);
                objParameters[1] = new SqlParameter("@LOCNAME", SqlDbType.VarChar);
                objParameters[2] = new SqlParameter("@PLANTCODE", SqlDbType.VarChar);
                objParameters[3] = new SqlParameter("@WHTYPE", SqlDbType.VarChar);
                ...
                objParameters[0].Value = objFields.strLocationID.Trim();
                objParameters[1].Value = objFields.strLocationName.Trim();
                objParameters[2].Value = objFields.strPlantCode.Trim();
                objParameters[3].Value = objFields.strWarehouseType.Trim();
                ...
                if (objSql.ExecuteProcedureParam(objSql.strLocal, "sp_INSERTBINMASTER", objParameters, "@RESULT", "@RESULT") != "")
                {
